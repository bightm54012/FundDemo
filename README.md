...
在圖表資料選取互動中，針對 Tooltip 的視覺呈現方式進行調整，以提升辨識度與整體 UI 一致性。

- **原設計**
  - Tooltip 以藍色邊框標示選取區塊。

- **優化後實作**
  - 移除藍色邊框，改以陰影（Shadow）效果呈現 Tooltip 區塊。

- **決策原因**
  - 圖表本身的折線採用藍色系，Tooltip 若同樣使用藍色邊框，容易與圖表線條產生視覺混淆。
  - 改用陰影效果後，可在不增加色彩干擾的情況下，清楚區隔 Tooltip 與圖表內容。
